...
export function ContactSection({ lang }: { lang: Language }) {
  ...
  return (
    <section id="contact" ref={ref} className={`relative py-16 md:py-20 pb-12 md:pb-16 px-4 md:px-6 transition-all duration-700 ${isInView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
      <div className="max-w-4xl mx-auto">
        ...
        <div className="max-w-2xl mx-auto space-y-8">
          <form action={FORMSPREE_URL} method="POST" onSubmit={() => { setIsLoading(true); setTimeout(() => { toast({ title: "¡Enviado!", description: text.form.success }); setIsLoading(false); }, 800); }} className="glass-card p-6 md:p-10 space-y-6">
            {/* tus inputs igual */}
            {/* ... */}
          ...
          {/* WhatsApp grande y premium */}
          <div className="flex justify-center">
            <a
              href="https://wa.me/5491138113906"
              target="_blank"
              rel="noopener noreferrer"
              className="group p-5 rounded-full bg-gray-800 hover:bg-[#25D366] border border-gray-700 hover:border-transparent transition-all duration-300 hover:scale-110 shadow-2xl"
            >
              <svg viewBox="0 0 24 24" className="w-10 h-10 text-gray-400 group-hover:text-white transition-colors" fill="currentColor">
          ...
          <p className="text-center text-gray-400 text-sm md:text-base">{text.disclaimer}</p>
        </div>
      </div>
    </section>
  );
